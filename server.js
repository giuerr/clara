/**
 * Clara — Operations Lead Agent (v6)
 *
 * Module structure:
 *   gmail-adapter.js    — Gmail/Google OAuth, email send/receive, MIME encoding
 *   calendar-adapter.js — Google Calendar CRUD, RSVP tracking, free slots
 *   telegram-handler.js — Telegram webhook, messaging, file vault integration
 *   crm-engine.js       — CRM profiles, sentiment, pipeline, contact resolution
 *   task-parser.js      — Claude AI integration, instruction parsing, reports
 *
 * This file (server.js) is the orchestrator: Express setup, route wiring,
 * shared state, polling loop, and scheduled jobs.
 */
...
const crmEngine        = require("./crm-engine");
...
const _ic = createInstitutionalCore({
  agentId:      'clara',
  agentVersion: '6.0.0',
  dataDir:      process.env.CLARA_DATA_DIR || path.join(__dirname, 'data'),
  scopeTopics:  [
    'data-room', 'lp-onboarding', 'compliance-calendar', 'scheduling',
    'email-management', 'crm', 'outreach',
  ],
  approvalRules: {
    'email_to_lp':          'review',
    'email_to_counterparty': 'review',
    'email_internal':       'auto',
    'email_to_owner':       'auto',
    'data_export':          'notify',
    'profile_update':       'auto',
...
// ── Load persisted OAuth tokens before anything else ────────────────────────
gmailAdapter.loadPersistedTokens();
...
// ─── Autonomy config ─────────────────────────────────────────────────────────
const AUTONOMOUS_MODE = true; // Clara handles routine requests without waiting for ${OWNER_NAME}
...
function advanceConversationState(email, event) {
  if (!email || isOwner(email) || email.toLowerCase() === CLARA_EMAIL.toLowerCase()) return;
...
const OWNER_DEFAULT    = process.env.OWNER_EMAIL || OWNER_EMAILS[0] || "";
...
const SNIPPET_IDENTITY =
  ...
  `CRITICAL: You are ${CLARA_NAME}, not Claude, not an AI assistant. Never introduce yourself as Claude or mention Anthropic. ` +
  `Never say 'I'm an AI' or 'as an AI'. You are a real person named ${CLARA_NAME}.`;
...
if (process.env.GOOGLE_CALENDAR_REFRESH_TOKEN) {
  ...
  // Fallback: use Clara's token until ${OWNER_NAME}'s is configured
  calendarOAuth2Client.setCredentials({ refresh_token: process.env.GOOGLE_REFRESH_TOKEN });
  addLog("⚠️ GOOGLE_CALENDAR_REFRESH_TOKEN not set — using Clara's token for calendar (organizer will show as Clara)", "warning");
}
// Monitor token events — alert on auth errors so Clara doesn't stop silently
...
async function withAuthCheck(fn) {
  try {
    ...
    if (e.code === 401 || e.code === 403 || (e.message && e.message.includes("invalid_grant"))) {
      ...
      addLog("🔐 Google auth error — token may have expired. Visit /auth/login to reauthorize.", "error");
    }
    throw e;
  }
}
...
const AUDIT_LOG_FILE          = path.join(DATA_DIR, "audit_log.json");           // never wiped — NANDA decision audit trail
if (!fs.existsSync(VAULT_DIR)) fs.mkdirSync(VAULT_DIR, { recursive: true });

// Atomic write — async by default, with sync fallback for shutdown
...
// Sync version only for shutdown/signal handlers
function atomicWriteSync(filePath, data) {
  const tmp = filePath + ".tmp";
  fs.writeFileSync(tmp, data);
  fs.renameSync(tmp, filePath);
}
// Default atomicWrite is now async — won't block the event loop
const atomicWrite = atomicWriteAsync;
...
let contacts            = cryptoStore.readEncrypted(CONTACTS_FILE, {}); // { "name_lower": { email, name, lastSeen } } — encrypted at rest
let profiles            = cryptoStore.readEncrypted(PROFILES_FILE, {}); // { "email": { ...CRM profile } } — encrypted at rest
let persistentRules     = loadJSON(RULES_FILE, []);    // [{ rule, addedAt }] — instructions from ${OWNER_NAME}
let scheduledQueue      = cryptoStore.readEncrypted(SCHEDULED_QUEUE_FILE, []); // [{ sendAt, to, subject, body, addedAt }] — encrypted at rest
let expenses            = cryptoStore.readEncrypted(EXPENSES_FILE, []);           // encrypted at rest
let rsvpStatus          = cryptoStore.readEncrypted(RSVP_FILE, {}); // { "eventId": { "email": "accepted"|... } } — encrypted at rest (contains attendee emails)
let crmDeleted          = new Set(cryptoStore.readEncrypted(CRM_DELETED_FILE, [])); // emails manually removed — encrypted at rest
let vaultIndex          = loadJSON(VAULT_INDEX_FILE, []);          // [{ id, filename, mimeType, size, savedAt, source, caption }]
let campaigns           = cryptoStore.readEncrypted(CAMPAIGNS_FILE, []);            // outreach campaigns — encrypted at rest

// ── Secure document links ────────────────────────────────────────────────────
const DOC_LINKS_FILE = path.join(DATA_DIR, "doc_links.json");
let docLinks = loadJSON(DOC_LINKS_FILE, []);
function saveDocLinks() { atomicWrite(DOC_LINKS_FILE, JSON.stringify(docLinks, null, 2)).catch(e => console.error(e.message)); }
...
function saveThreads()        { atomicWrite(THREADS_FILE, JSON.stringify(activeThreads, null, 2)).catch(e => console.error(e.message)); }
function saveProcessedIds() {
  ...
  atomicWrite(PROCESSED_IDS_FILE, JSON.stringify([...processedMessageIds])).catch(e => console.error(e.message));
}
function _saveContactsNow()   { cryptoStore.atomicWriteEncrypted(CONTACTS_FILE, contacts).catch(e => console.error(e.message)); }
function _saveProfilesNow()   { cryptoStore.atomicWriteEncrypted(PROFILES_FILE, profiles).catch(e => console.error(e.message)); }
...
function saveRules()          { atomicWrite(RULES_FILE, JSON.stringify(persistentRules, null, 2)).catch(e => console.error(e.message)); }
function saveScheduledQueue() { cryptoStore.atomicWriteEncrypted(SCHEDULED_QUEUE_FILE, scheduledQueue).catch(e => console.error(e.message)); }
function saveExpenses()       { cryptoStore.atomicWriteEncrypted(EXPENSES_FILE, expenses).catch(e => console.error(e.message)); }
function saveRsvpStatus()     { cryptoStore.atomicWriteEncrypted(RSVP_FILE, rsvpStatus).catch(e => console.error(e.message)); }
function saveCrmDeleted()     { cryptoStore.atomicWriteEncrypted(CRM_DELETED_FILE, [...crmDeleted]).catch(e => console.error(e.message)); }
function saveVaultIndex()     { atomicWrite(VAULT_INDEX_FILE, JSON.stringify(vaultIndex, null, 2)).catch(e => console.error(e.message)); }
function saveCampaigns()      { cryptoStore.atomicWriteEncrypted(CAMPAIGNS_FILE, campaigns).catch(e => console.error(e.message)); }

// ─── NANDA: Security & Attestation — Decision Audit Log ──────────────────────
// Records every consequential action Clara takes (emails sent, calendar ops,
// outreach, profile changes) with the reasoning context for post-hoc review.
let auditLog = cryptoStore.readEncrypted(AUDIT_LOG_FILE, []); // encrypted at rest (contains PII targets)
function auditDecision({ action, target, detail, reasoning, source }) {
  const entry = {
    id: `AUD-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`,
    timestamp: new Date().toISOString(),
    agent: 'Clara',
    agentVersion: '6.0.0',
    action,           // e.g. 'email_sent', 'calendar_event_created', 'outreach_drafted', 'profile_updated'
    target,           // e.g. email address, event ID, campaign ID
    detail,           // human-readable summary of what happened
    reasoning,        // why the agent took this action (instruction match, owner request, autonomous decision)
    source,           // 'telegram', 'email_poll', 'dashboard', 'scheduled', 'autonomous'
  };
  auditLog.push(entry);
  // Persistent JSONL audit via institutional-core
  _ic.audit.log({
    action:   entry.action,
    module:   'clara',
    input:    entry.target || '',
    output:   entry.detail || '',
    metadata: { reasoning: entry.reasoning, source: entry.source },
  });
  // Cap at 2000 entries, trim oldest
  if (auditLog.length > 2000) auditLog = auditLog.slice(-2000);
  // Debounced save
  if (!auditDecision._timer) {
    auditDecision._timer = setTimeout(() => {
      auditDecision._timer = null;
      cryptoStore.atomicWriteEncrypted(AUDIT_LOG_FILE, auditLog).catch(e => console.error(e.message));
    }, 1000);
  }
}
auditDecision._timer = null;
...
function vaultLoad(entry) {
  if (!entry?.diskPath || !fs.existsSync(entry.diskPath)) return null;
...
function sanitiseRecipient(email) {
  ...
  if (!VALID_EMAIL_RE.test(lower)) return null;
  if (isOwner(lower) || lower === CLARA_EMAIL.toLowerCase()) return null;
  return lower;
}

// ─── LRU eviction — prevent unbounded memory growth ─────────────────────────
function evictOldData() {
  // Cap activeThreads: keep only last 500 active + most recent 200 done
  const entries = Object.entries(activeThreads);
  if (entries.length > 700) {
    const active = entries.filter(([,t]) => t.stage !== "done" && t.stage !== "cancelled");
    const done = entries.filter(([,t]) => t.stage === "done" || t.stage === "cancelled")
      .sort((a, b) => new Date(b[1].lastContact || 0) - new Date(a[1].lastContact || 0))
      .slice(0, 200);
    activeThreads = Object.fromEntries([...active, ...done]);
    addLog(`🧹 LRU: trimmed threads from ${entries.length} to ${Object.keys(activeThreads).length}`, "info");
    saveThreads();
  }

  // Cap profiles: keep only last 1000 by lastContact
  const profileEntries = Object.entries(profiles);
  if (profileEntries.length > 1000) {
    const sorted = profileEntries.sort((a, b) => new Date(b[1].lastContact || 0) - new Date(a[1].lastContact || 0));
    profiles = Object.fromEntries(sorted.slice(0, 1000));
    addLog(`🧹 LRU: trimmed profiles from ${profileEntries.length} to 1000`, "info");
    saveProfiles();
  }

  // Cap crmDeleted: keep only last 500
  if (crmDeleted.size > 500) {
    const arr = [...crmDeleted];
    crmDeleted = new Set(arr.slice(-500));
    saveCrmDeleted();
  }
...
// Run eviction on startup and then hourly
evictOldData();
const _evictionInterval = setInterval(evictOldData, 60 * 60 * 1000);
if (_evictionInterval.unref) _evictionInterval.unref();

// ─── Context object — shared state bridge for all modules ───────────────────
const ctx = {
  // Constants
  OWNER_NAME, OWNER_DEFAULT, OWNER_CALENDAR, OWNER_PHONE, OWNER_EMAILS,
  CLARA_NAME, CLARA_EMAIL, CLARA_SIGNATURE, DASHBOARD_PASSWORD,
  TELEGRAM_ENABLED, TELEGRAM_TOKEN, AUTONOMOUS_MODE: true,
  get TELEGRAM_CHAT_ID() { return TELEGRAM_CHAT_ID; },
  set TELEGRAM_CHAT_ID(v) { TELEGRAM_CHAT_ID = v; },

  // Config
  config,

  // State (getters for live mutable references)
  get profiles() { return profiles; },
  set profiles(v) { profiles = v; },
  get activeThreads() { return activeThreads; },
  set activeThreads(v) { activeThreads = v; },
  get contacts() { return contacts; },
  get expenses() { return expenses; },
  get persistentRules() { return persistentRules; },
  get rsvpStatus() { return rsvpStatus; },
  set rsvpStatus(v) { rsvpStatus = v; },
  get crmDeleted() { return crmDeleted; },
  get vaultIndex() { return vaultIndex; },
  get campaigns() { return campaigns; },
  get processedMessageIds() { return processedMessageIds; },
  get logs() { return logs; },
  get scheduledQueue() { return scheduledQueue; },

  // Save functions
  addLog, saveProfiles, saveContacts, saveThreads, saveRules,
  saveScheduledQueue, saveExpenses, saveRsvpStatus, saveCrmDeleted,
  saveVaultIndex, saveCampaigns, saveThread, saveThreadsDebounced,
  saveProcessedIds, saveDocLinks, saveCampaigns,

  // Utility
  isOwner, isWithinActiveHours, ownerGreeting, truncate, parseJSON,
  safeOwnerEmail, sanitiseRecipient, firstNameOnly, learnContact,
  findThread, findThreadByEmail, vaultSave, vaultFind, vaultLoad,
  auditDecision, requireAuth, pdfParse,
  VALID_EMAIL_RE,

  // Model constants (from task-parser)
  MODEL_HAIKU: taskParser.MODEL_HAIKU,
  MODEL_FAST: taskParser.MODEL_FAST,
  MODEL_CAPABLE: taskParser.MODEL_CAPABLE,
};

// ── Initialize modules with ctx ─────────────────────────────────────────────
gmailAdapter.init(ctx);

// Populate ctx with cross-module function references
ctx.getOAuth2Client = gmailAdapter.getOAuth2Client;
ctx.getCalendarOAuth2Client = gmailAdapter.getCalendarOAuth2Client;
ctx.getGmail = gmailAdapter.getGmail;
ctx.getCalendar = gmailAdapter.getCalendar;

// Task parser functions
const snippets = taskParser.buildSnippets(ctx);
ctx.SNIPPET_IDENTITY = snippets.SNIPPET_IDENTITY;
ctx.SNIPPET_TONE = snippets.SNIPPET_TONE;
ctx.SNIPPET_OWNER_TONE = snippets.SNIPPET_OWNER_TONE;
ctx.SNIPPET_THIRD_PARTY_TONE = snippets.SNIPPET_THIRD_PARTY_TONE;
ctx.SNIPPET_LANGUAGE = snippets.SNIPPET_LANGUAGE;
ctx.SNIPPET_GREETINGS = snippets.SNIPPET_GREETINGS;
ctx.SNIPPET_SCHEDULING = snippets.SNIPPET_SCHEDULING;
ctx.SNIPPET_DRAFT = snippets.SNIPPET_DRAFT;
ctx.SNIPPET_OWNER_REPLY = snippets.SNIPPET_OWNER_REPLY;
ctx.MSG_STYLE = snippets.MSG_STYLE;

// Bind module functions to ctx for cross-module use
ctx.askClaude = (prompt, maxTokens, retries, model) => taskParser.askClaude(ctx, prompt, maxTokens, retries, model);
ctx.askClaudeWithWebSearch = (prompt, opts) => taskParser.askClaudeWithWebSearch(ctx, prompt, opts);
ctx.askClaudeVision = (contentBlocks, maxTokens, model) => taskParser.askClaudeVision(ctx, contentBlocks, maxTokens, model);
ctx.wrapUntrusted = taskParser.wrapUntrusted;
ctx.detectLanguage = (text) => taskParser.detectLanguage(ctx, text);
ctx.sanitiseLang = taskParser.sanitiseLang;
ctx.localSig = (lang) => taskParser.localSig(ctx, lang);
ctx.withRules = (snippet) => taskParser.withRules(snippet, persistentRules);
ctx.buildReportDocx = taskParser.buildReportDocx;
ctx.parseInstructions = (body, subject) => taskParser.parseInstructions(ctx, body, subject);

ctx.sendEmail = (opts) => gmailAdapter.sendEmail(ctx, opts);
ctx.sendTelegram = (chatId, msg) => telegramHandler.sendTelegram(ctx, chatId, msg);
ctx.alertOwner = (msg) => telegramHandler.alertOwner(ctx, msg);
ctx.enrichProfile = (email, data) => crmEngine.enrichProfile(ctx, email, data);
ctx.getProfileContext = (email) => crmEngine.getProfileContext(ctx, email);
ctx.resolveEmailForName = (name, hint, emails) => crmEngine.resolveEmailForName(ctx, name, hint, emails);
ctx.resolveRecipientEmails = (tasks, body, emails) => crmEngine.resolveRecipientEmails(ctx, tasks, body, emails);
ctx.advanceConversationState = (email, event) => crmEngine.advanceConversationState(ctx, email, event);
ctx.advancePipeline = (email, reason) => crmEngine.advancePipeline(ctx, email, reason);

ctx.fetchCalendarEvents = (opts) => calendarAdapter.fetchCalendarEvents(ctx, opts);
ctx.formatCalendarEvents = (events) => calendarAdapter.formatCalendarEvents(ctx, events);
ctx.createCalendarEvent = (opts) => calendarAdapter.createCalendarEvent(ctx, opts);
ctx.updateCalendarEvent = (opts) => calendarAdapter.updateCalendarEvent(ctx, opts);
ctx.cancelCalendarEvent = (opts) => calendarAdapter.cancelCalendarEvent(ctx, opts);
ctx.findCalendarEventId = (email, name) => calendarAdapter.findCalendarEventId(ctx, email, name);
ctx.findFreeSlots = (dur, days) => calendarAdapter.findFreeSlots(ctx, dur, days);
ctx.bookConfirmedMeeting = (t, time) => calendarAdapter.bookConfirmedMeeting(ctx, t, time);
ctx.checkCalendarRSVPs = () => calendarAdapter.checkCalendarRSVPs(ctx);
ctx.parseTime = (str) => calendarAdapter.parseTime(ctx, str);

// Gmail helpers
ctx.extractEmail = gmailAdapter.extractEmail;
ctx.getTextBody = gmailAdapter.getTextBody;
ctx.getNameForEmail = gmailAdapter.getNameForEmail;
ctx.extractFirstName = gmailAdapter.extractFirstName;
ctx.getAttachmentSummary = gmailAdapter.getAttachmentSummary;
ctx.getAttachmentParts = gmailAdapter.getAttachmentParts;
ctx.fetchAttachmentData = gmailAdapter.fetchAttachmentData;
ctx.cleanSubject = gmailAdapter.cleanSubject;

// Follow-up and template modules
ctx.followUp = followUp;
ctx.templates = templates;

// Initialize follow-up and template modules
templates.init(ctx);
followUp.init(ctx);

// ── Wire executeTask + handleInboundMessage onto ctx (set after they're defined below) ──
// These are set later since they reference functions defined further down in server.js
ctx.executeTask = null;
ctx.handleInboundMessage = null;

addLog("✅ All modules initialized and wired via ctx", "success");
...
async function enrichProfile(email, { name, direction, subject, body }) {
  ...
  try {
    const raw = await askClaude(
      `${SNIPPET_IDENTITY}\nYou are analysing an email to build a CRM profile for ${OWNER_NAME}'s Operations Lead Clara.\n\n` +
      `Contact email: ${key}\nKnown name: ${existing.name || "unknown"}\n` +
      `Email direction: ${direction} (sent = Clara/${OWNER_NAME} sent to this person; received = this person wrote to Clara)\n` +
...
async function bootstrapProfiles() {
  ...
  try {
    // Fetch last 6 months of sent + received emails addressed to/from Clara
    const sixMonthsAgo = Math.floor((Date.now() - 180 * 24 * 60 * 60 * 1000) / 1000);
    const queries = [
      `from:${CLARA_EMAIL} after:${sixMonthsAgo}`,
      `to:${CLARA_EMAIL} after:${sixMonthsAgo}`,
    ];
    ...
    for (const id of idsToProcess) {
      try {
        ...
        const isFromClara = fromAddr === CLARA_EMAIL.toLowerCase();
        if (isFromClara) {
          // Sent email — enrich profile for each recipient
          const toAddrs = (toRaw.match(/[\w.+\-]+@[\w.\-]+\.\w+/g) || [])
            .filter(e => !isOwner(e) && e.toLowerCase() !== CLARA_EMAIL.toLowerCase());
...
(function cleanupProfiles() {
  ...
  for (const key of Object.keys(profiles)) {
    if (isCrmBlocked(key) || isOwner(key) || key === CLARA_EMAIL.toLowerCase()) {
      delete profiles[key];
      purged++;
    }
  }
  ...
  // 5. Clean up old completed/cancelled threads (older than 30 days)
  ...
  if (archivedThreads) addLog(`🗄️ Archived ${archivedThreads} old thread(s) (>30 days)`, "info");
  ...
  if (possibleDupes.length && TELEGRAM_ENABLED) {
    ...
    setTimeout(async () => {
      ...
      await sendTelegram(TELEGRAM_CHAT_ID, dedupMsg).catch(e => addLog(`⚠️ Silent failure: ${e.message}`, "warning"));
...
const INJECTION_GUARD_SYSTEM =
  `You are ${CLARA_NAME}'s processing engine. ` +
        ...
        for (const m of res.data.messages || []) {
          ...
          for (const h of msg.data.payload.headers) {
            ...
            for (const addr of found) {
              ...
              if (!isOwner(lower) && lower !== CLARA_EMAIL.toLowerCase()) candidates.add(lower);
            }
          }
        }
      } catch { /* non-fatal */ }
...
async function localSig(language) {
  ...
  if (key === "english") return CLARA_SIGNATURE;
...
${CLARA_SIGNATURE}`, 120, 1, MODEL_HAIKU)).trim();
    sigCache.set(key, translated);
    return translated;
  } catch { return CLARA_SIGNATURE; }
}
...
async function sendEmail({ to, subject, body, threadId, inReplyTo, references, attachment, attachments, cc = null, fromOwner = null, ignoreHours = false }) {
  ...
  // Institutional-core: approval gate check before sending
  if (!toOwner) {
    const approvalType = toAddr === CLARA_EMAIL?.toLowerCase() ? 'email_internal' : 'email_to_counterparty';
    const approvalCheck = _ic.approval.check(approvalType, { recipient: toAddr });
    if (!approvalCheck.approved) {
      _ic.audit.log({ action: 'email_blocked_by_approval_gate', module: 'clara', input: toAddr, output: approvalCheck.message });
      addLog(`🚫 Email to ${toAddr} blocked by approval gate (tier: ${approvalCheck.tier}): ${approvalCheck.message}`, "warning");
      return { blocked: true, pendingId: approvalCheck.pendingId, message: approvalCheck.message, tier: approvalCheck.tier };
    }
  }
  const sent = await gmail.users.messages.send({ userId: "me", requestBody: { raw, threadId } });
  addLog(`📤 Sent to ${to}: "${subject}"`, "success");
  // NANDA audit: log every email sent
  auditDecision({
    action: 'email_sent',
    target: toAddr,
  ...
  // Version the sent email (best-effort)
  try {
    if (_ic && _ic.versioning) {
      _ic.versioning.save(
        `email-${toAddr}-${Date.now()}`.replace(/[^a-zA-Z0-9_-]/g, '-'),
        JSON.stringify({ to: toAddr, subject, body, sentAt: new Date().toISOString() }),
        { action: 'email_sent', userId: 'clara' }
      );
    }
  } catch (_verr) { /* versioning is best-effort */ }
...
async function createCalendarEvent({ summary, startDateTime, endDateTime, attendees, description, isPhoneCall, isInPerson, isGoogleMeet, location }) {
  ...
  // NANDA audit: calendar event creation
  auditDecision({
...
async function updateCalendarEvent({ eventId, startDateTime, endDateTime }) {
  ...
  // NANDA audit: calendar event update
  auditDecision({
...
async function cancelCalendarEvent({ eventId }) {
  ...
  // NANDA audit: calendar event cancellation
  auditDecision({
...
function formatAttendeeRSVP(attendees) {
  return (attendees || [])
    .filter(a => !isOwner(a.email) && a.email.toLowerCase() !== CLARA_EMAIL.toLowerCase())
...
// Returns a human-readable string of 3-5 available slots in the next 7 working days
async function findFreeSlots(durationMinutes = 60, daysAhead = 7) {
  try {
    ...
    // Walk through working hours (09:00–18:00 Rome time) day by day
    const freeSlots = [];
    const cursor = new Date(now);
    cursor.setMinutes(Math.ceil(cursor.getMinutes() / 30) * 30, 0, 0); // round up to next 30min
    ...
    while (freeSlots.length < 5 && cursor < end) {
      ...
      if (!clash) {
        ...
        cursor.setTime(slotEnd.getTime()); // jump past this slot
      } else {
...
async function checkCalendarRSVPs() {
  try {
    ...
    for (const ev of events) {
      const attendees = (ev.attendees || []).filter(a => !isOwner(a.email) && a.email.toLowerCase() !== CLARA_EMAIL.toLowerCase());
    ...
    if (changes.length) {
      ...
      if (declines.length && TELEGRAM_ENABLED && TELEGRAM_CHAT_ID) {
        await sendTelegram(TELEGRAM_CHAT_ID, `📅 ${declines.map(c => `${c.name} declined "${c.event}" (${c.time})`).join("; ")}`).catch(e => addLog(`⚠️ Silent failure: ${e.message}`, "warning"));
      }
    }
...
${CLARA_SIGNATURE}`,
        ...
        await alertOwner(`⚠️ Clara: Conflict when booking ${calDisplayName} at ${confirmedTime} — you already have ${existing[0].summary || "another meeting"}`);
      }
  ...
  // Persist the confirmed contact so Clara remembers them
...
async function parseInstructions(body, subject) {
  // Institutional-core: scope guard — warn if message is outside Clara's domain
  if (_ic) {
    const scopeResult = _ic.scopeGuard.check(body);
    if (!scopeResult.inScope) {
      addLog(`⚠️ Scope guard: "${scopeResult.topic}" → suggested agent: ${scopeResult.suggestedAgent}`, "warning");
    }
  }
  const raw = await askClaude(`Parse ${OWNER_NAME}'s instructions to his Operations Lead Clara. ${OWNER_NAME} may write in any language — understand the meaning regardless. Return a JSON array of task objects.
...
- DIRECT_CALENDAR_INVITE: ${OWNER_NAME} wants to send a calendar invite directly — he provides the recipient AND an exact date/time. Trigger phrases include: "send a calendar invite", "send the invite", "send them the calendar link", "create a calendar event", "book at [time]", "set up a call at [time]", "meeting at [time]", "send the link for [time]", or ANY instruction where the owner gives a specific date/time for a meeting. Use "body" for the confirmed time (e.g. "today at 3pm", "Thursday 25 March at 10:00"). Set isPhoneCall/isInPerson/isGoogleMeet in "note" if specified. IMPORTANT: if ${OWNER_NAME} provides a time, ALWAYS use DIRECT_CALENDAR_INVITE — do NOT use BOOK_MEETING or BOOK_PHONE_CALL. The calendar invite is sent immediately without asking the other party for availability.
- BOOK_MEETING: schedule a meeting with someone when NO specific time is given and the owner does NOT say "send the calendar link" or "send the invite". This kicks off a scheduling thread to find a mutually convenient time. Only use this when ${OWNER_NAME} says "find a time", "schedule a meeting", "set up a meeting" WITHOUT providing a concrete date/time.
- BOOK_PHONE_CALL: schedule a phone call with someone when NO specific time is given yet. Same rules as BOOK_MEETING but for phone calls.
...
- DAILY_SUMMARY: ${OWNER_NAME} wants a summary of what Clara has done today and what is scheduled in the next 24 hours. Use when he asks things like "what have you done today?", "give me a daily summary", "what's on for the next 24 hours?", "summary of today's activity".
- OUTREACH_SUMMARY: ${OWNER_NAME} wants a summary of who Clara has reached out to, their status, and any pending responses. Use when the owner asks things like "who have you reached out to?", "what's the status of your outreach?", "summarise the people you contacted", "give me an update on the meetings".
- CANCEL_OUTREACH: cancel/abort an active outreach thread (before a meeting is booked) — stop following up with that person. Use when ${OWNER_NAME} says "cancel the outreach to X", "stop the scheduling with Y", "forget about the meeting with Z". Use "recipients" for the person's name/email.
...
- REMEMBER: ${OWNER_NAME} wants Clara to permanently remember a rule or preference (e.g. "Clara, always check with me before accepting meetings with X", "remember that Marco prefers morning calls", "from now on always cc me when writing to Y"). Store the rule exactly as stated.
...
- SEND_FILE: ${OWNER_NAME} wants to send a file/document/deck/attachment that he previously shared with Clara (via Telegram or email) to someone. Use "recipients" for the target, "note" for which file to send (filename or description), "body" for any accompanying message.
...
async function handleOwnerQuery({ fromAddress, subject, body, messageId, gmailThreadId, ownerLang = "English" }) {
  ...
  try {
    const threadSummary = Object.entries(activeThreads)
      .filter(([, t]) => t.stage !== "done" && t.stage !== "cancelled")
      .map(([, t]) => `- ${t.thirdPartyFirstName || t.restaurantName || "?"} (${t.thirdPartyEmail || ""}), stage=${t.stage}, subject="${t.originalSubject || ""}"`)
      .join("\n") || "No active threads.";
    ...
    const relevantThreadSummary = mentionedNames.length > 0
      ? Object.entries(activeThreads)
          ...
          .map(([, t]) => `- ${t.thirdPartyFirstName || "?"} (${t.thirdPartyEmail || ""}), stage=${t.stage}, subject="${t.originalSubject || ""}"`)
    ...
    const answer = await askClaude(
      `You are ${CLARA_NAME}, PA to ${OWNER_NAME}. ${OWNER_NAME} asked a question.\n\n` +
...
async function executeTask(task, { fromAddress, subject: origSubject, body: origBody, messageId, gmailThreadId, ownerLang = "English", attachmentParts = [] }) {
  const taskSubject = task.subject || cleanSubject(origSubject);
  ...
  if (task.type === "SEND_EMAIL") {
    ...
    if (!task._confirmed) {
      // Always draft for review — ${OWNER_NAME} sees every outbound email before it goes
      {
        const drafts = [];
        for (const r of recipients) {
          ...
          const draftBody = await askClaude(`${withRules(SNIPPET_DRAFT)}${draftHint}\n\nDraft an email on behalf of ${OWNER_NAME}.\nRecipient: ${r.name || to} <${to}>${r.personalNote ? "\nPersonal note: " + wrapUntrusted(r.personalNote) : ""}\nInstructions: ${wrapUntrusted(task.body || "write as appropriate")}${task.note ? "\nNote: " + wrapUntrusted(task.note) : ""}\nWrite email body only. Sign off as Clara.`);
          // Institutional-core: confidence scoring on draft
          const scored = _ic.confidence.score(draftBody, { withinCoreDomain: true, allInputsProvided: true });
          if (scored.confidence.level === 'LOW' || scored.confidence.level === 'INSUFFICIENT') {
            _ic.audit.log({ action: 'low_confidence_output', module: 'clara', output: `Confidence: ${scored.confidence.score} (${scored.confidence.level})` });
          }
  ...
  if (task.type === "FORWARD_ATTACHMENT") {
    ...
    if (!partsToSend.length) {
      await sendEmail({ to: fromAddress, subject: `Re: ${origSubject}`, body: `${ownerGreeting()}\n\nI couldn't find any attachments in your email to forward. Could you check and resend?\n\n${CLARA_SIGNATURE}`, threadId: gmailThreadId, inReplyTo: messageId, references: messageId });
  ...
  if (task.type === "DIRECT_CALENDAR_INVITE") {
    ...
    for (const r of recipients) {
      ...
      if (!confirmedTime) {
        await sendEmail({ to: fromAddress, subject: `Re: ${origSubject}`, body: `${ownerGreeting()}\n\nI'd like to create the invite with ${name}, but I'm not sure what time you'd like. Could you let me know the date and time?\n\n${CLARA_SIGNATURE}`, threadId: gmailThreadId, inReplyTo: messageId, references: messageId });
  ...
  if (task.type === "BOOK_MEETING" || task.type === "BOOK_PHONE_CALL") {
    ...
    const slotCheck = await askClaude(`${OWNER_NAME} asked Clara to schedule a meeting. Did he provide specific time slots to offer?\nMessage: ${wrapUntrusted(task.body || origSubject)}\nReply with: SLOTS: [the slots] or NO_SLOTS`, 64, 1, MODEL_HAIKU);
    const hasSlots  = slotCheck.startsWith("SLOTS:");
    const slots     = hasSlots ? slotCheck.replace("SLOTS:", "").trim() : null;
...
${SNIPPET_SCHEDULING}\n\nWrite a short, warm email to ${name} on behalf of ${OWNER_NAME} proposing these time slots for a ${isPhoneCall ? "phone call" : "meeting"}: "${slots}". Ask which works best.\nOpening: Dear ${name},\nWrite in English\nClosing: ${sig}\nWrite email body only`, 400, 1, MODEL_FAST);
        ...
        saveThread(threadId, { stage: "waiting_for_confirmation", thirdPartyEmail: email, thirdPartyFirstName: name, originalSubject: `Meeting with ${OWNER_NAME}`, lastThirdPartyMessageId: sent?.id, thirdPartyGmailThreadId: threadId, ownerEmail: safeOwnerEmail(fromAddress), ownerGmailThreadId: gmailThreadId, isPhoneCall, isFirstContact: true, triggeredByOwner: true, slotsOffered: slots, ownerConfirmed: true, thirdPartyConfirmed: false, note: task.note || null });
...
${SNIPPET_SCHEDULING}\n\nWrite a short, warm email to ${name} on behalf of ${OWNER_NAME} to find a time for a ${isPhoneCall ? "phone call" : "meeting"}. Ask what times work for them.${task.note ? "\nContext: " + wrapUntrusted(task.note) : ""}\nOpening: Dear ${name},\nWrite in English\nClosing: ${sig}\nWrite email body only`, 400, 1, MODEL_FAST);
        ...
        saveThread(threadId, { stage: autoConfirmed ? "waiting_for_confirmation" : "waiting_for_slots", thirdPartyEmail: email, thirdPartyFirstName: name, originalSubject: `Meeting with ${OWNER_NAME}`, lastThirdPartyMessageId: sent?.id, thirdPartyGmailThreadId: threadId, ownerEmail: safeOwnerEmail(fromAddress), ownerGmailThreadId: gmailThreadId, isPhoneCall, isFirstContact: true, triggeredByOwner: true, thirdPartyAskedForSlots: !autoConfirmed, slotsOffered: autoSlots || null, ownerConfirmed: autoConfirmed, thirdPartyConfirmed: false, note: task.note || null });
        bookSent.push(name);
  ...
  if (task.type === "RESEARCH") {
    ...
    try {
      ...
      await sendEmail({ to: fromAddress, subject: `Report: ${taskSubject}`, body: `${ownerGreeting()}\n\nPlease find the report attached.\n\n${CLARA_SIGNATURE}`, threadId: gmailThreadId, inReplyTo: messageId, references: messageId, attachment: { filename: safeFilename, contentType: "application/vnd.openxmlformats-officedocument.wordprocessingml.document", buffer: docxBuffer } });
  ...
  // The owner asks Clara to contact someone to propose moving their meeting
...
${CLARA_SIGNATURE}`, threadId: gmailThreadId, inReplyTo: messageId, references: messageId });
  ...
  if (task.type === "EMAIL_DIGEST") {
    try {
      // Fetch recent unread emails from ${OWNER_NAME}'s inbox (not from Clara or the owner)
      const since = new Date(Date.now() - 24 * 60 * 60 * 1000); // last 24h
      const sinceUnix = Math.floor(since.getTime() / 1000);
      const res = await gmail.users.messages.list({
        userId: "me", q: `is:unread after:${sinceUnix} -from:${CLARA_EMAIL} -from:me`, maxResults: 20,
      });
...
${CLARA_SIGNATURE}`, threadId: gmailThreadId, inReplyTo: messageId, references: messageId });
      ...
      const digestBody = await askClaude(
        `You are ${CLARA_NAME}, PA to ${OWNER_NAME}. Summarise these unread emails concisely.\n\n` +
...
${lines}`;
    ...
    const summary = await askClaude(
      `You are ${CLARA_NAME}, PA to ${OWNER_NAME}. Write a clear expense summary.\n\n` +
      `Logged expenses:\n${sections}\n\n` +
  ...
  if (task.type === "DAILY_SUMMARY") {
    ...
    // ── What Clara has done today (activity log) ─────────────────────────────
    ...
    const summary = await askClaude(
      `You are ${CLARA_NAME}, PA to ${OWNER_NAME}. Write a clear, natural daily activity summary.\n\n` +
...
${CLARA_SIGNATURE}`, threadId: gmailThreadId, inReplyTo: messageId, references: messageId });
    ...
    if (active.filter(t => t.stage !== "done").length)
      sections.push("Active outreach:\n" + active.filter(t => t.stage !== "done").map(fmt).join("\n"));
    if (done.length)
      sections.push("Meetings booked:\n" + done.map(fmt).join("\n"));
    if (cancelled.length)
      sections.push("Cancelled:\n" + cancelled.map(fmt).join("\n"));

    const summaryBody = await askClaude(
      `You are ${CLARA_NAME}, PA to ${OWNER_NAME}. Write a clear, natural outreach status update.\n\n` +
      `Data:\n${sections.join("\n\n")}\n\n` +
  ...
  if (task.type === "CAMPAIGN_STATUS") {
    ...
    const statusBody = await askClaude(
      `You are ${CLARA_NAME}, PA to ${OWNER_NAME}. Write a clear campaign status update.\n\n` +
      `Data:\n${campSections.join("\n\n")}\n\n` +
  ...
  if (task.type === "PIPELINE_SUMMARY") {
    ...
    const summaryBody = await askClaude(
      `You are ${CLARA_NAME}, PA to ${OWNER_NAME}. Write a clear pipeline status update email.\n\n` +
  ...
  if (task.type === "CANCEL_OUTREACH") {
    ...
    const matches = Object.entries(activeThreads).filter(([, t]) => {
...
I couldn't find any active outreach thread for ${recipientName || recipientEmail || "that person"}. They may have already been cancelled or no outreach was sent.
...
Done — I've cancelled the outreach to ${cancelledNames.join(", ")} and stopped any follow-ups.${matches.some(([,t]) => t.calendarEventId) ? " I've also removed the calendar invite." : ""}
...
${CLARA_SIGNATURE}`, threadId: gmailThreadId, inReplyTo: messageId, references: messageId });
  ...
  if (task.type === "SEND_FILE") {
    ...
    // If no match by hint, try the most recent file
  ...
  if (task.type === "CANCEL_MEETING") {
    ...
    if (candidateEvents.length === 1 && !isBulkCancel) {
      ...
      try {
        ...
        await sendEmail({ to: fromAddress, subject: `Re: ${origSubject}`, body: `${ownerGreeting()}\n\nDone — I've cancelled the meeting "${ev.summary}" on ${new Date(ev.start?.dateTime || ev.start?.date).toLocaleString("en-GB", { weekday: "short", day: "numeric", month: "short", hour: "2-digit", minute: "2-digit", timeZone: "Europe/Rome" })}.\n\n${CLARA_SIGNATURE}`, threadId: gmailThreadId, inReplyTo: messageId, references: messageId });
...
async function handleOwnerInstruction({ fromAddress, subject, body, messageId, gmailThreadId, attachments = [], attachmentParts = [], ownerLang = "English", threadContext = "" }) {
  ...
  // Surface attachments immediately so the owner knows Clara saw them
  ...
  // This lets Clara link "reach out to Joshua" to "business@example.com" found in the forwarded email.
  const embeddedEmails = [...new Set((body.match(/[\w.+\-]+@[\w.\-]+\.\w+/g) || [])
    .map(e => e.toLowerCase())
    .filter(e => !isOwner(e) && e !== CLARA_EMAIL.toLowerCase()))];
  ...
  if (!rawTasks?.length) {
    ...
    const reply = await askClaude(`${withRules(SNIPPET_OWNER_REPLY)}\n\n${OWNER_NAME} sent this message:\n${wrapUntrusted(truncate(body, 2000))}\n\nReply naturally in ${ownerLang} — match the language he wrote in exactly. Write email body only.`, 400, 1, MODEL_FAST);
    ...
    try {
      ...
      if (result.detail === "draft_pending" && result._drafts?.length) {
        ...
        await sendEmail({
          ...
          body: `${ownerGreeting()}\n\nThis email looks important, so I've drafted it for your review before sending.\n\nReply "send it" to confirm, or make any changes you'd like.\n\n${result._draftPreview}\n\n${CLARA_SIGNATURE}`,
  ...
  const selfReporting  = new Set(["RESEARCH", "BOOKING", "LOOKUP", "BOOK_MEETING", "BOOK_PHONE_CALL", "DIRECT_CALENDAR_INVITE", "QUERY", "CALENDAR_QUERY", "RESCHEDULE_MEETING", "REACH_OUT_RESCHEDULE", "CANCEL_MEETING", "REMEMBER", "SCHEDULED_SEND", "DAILY_SUMMARY", "OUTREACH_SUMMARY", "CANCEL_OUTREACH", "FORWARD_ATTACHMENT", "SEND_FILE", "SHARE_DOCUMENT", "EXPENSE_SUMMARY", "EMAIL_DIGEST", "SET_TONE", "PIPELINE_SUMMARY", "CREATE_CAMPAIGN", "CAMPAIGN_STATUS", "LP_UPDATE", "CREATE_EVENT_CAMPAIGN"]);
  ...
  if (needsSummary) {
    ...
    const summaryBody = await askClaude(
      `You are ${CLARA_NAME}, PA to ${OWNER_NAME}. Write a brief, natural update email to the owner.\n\n` +
      `What happened:\n${contextParts.join("\n\n")}\n\n` +
      `Rules:\n` +
...
async function handleBookingTask({ fromAddress, subject, body, messageId, gmailThreadId }) {
  ...
  try {
    ...
    const bookingBody = [
      ...
      ``, `Please confirm availability at your earliest convenience.`, ``, CLARA_SIGNATURE,
    ].filter(Boolean).join("\n");
...
async function handleMessage(message, { withinHours = true } = {}) {
  ...
  // This ensures that when the owner replies "do the outreach", Clara can see
  ...
  if (!`${toRaw} ${ccRaw}`.toLowerCase().includes(CLARA_EMAIL.toLowerCase())) {
    addLog(`⏭️ Clara not addressed — skipping`); return;
  }
  ...
  if (!isOwner(fromAddress) && fromAddress !== CLARA_EMAIL.toLowerCase()) {
  ...
  if (isOwner(fromAddress)) {
    ...
    // ── Email toggle: "Clara stop" / "Clara start" ──────────────────────────
    const toggleText = `${subject} ${body}`.toLowerCase();
    const wantsStop  = /\bclara\s+stop\b|\bstop\s+clara\b/i.test(toggleText);
    const wantsStart = /\bclara\s+start\b|\bstart\s+clara\b/i.test(toggleText);

    if (wantsStop && isPolling) {
      stopPolling();
      await sendEmail({ to: fromAddress, subject: `Re: ${subject}`, body: `${ownerGreeting()}\n\nStopped. I won't process any emails until you tell me to start again.\n\n${CLARA_SIGNATURE}`, threadId: gmailThreadId, inReplyTo: messageId, references: messageId });
      addLog(`⏹️ Clara stopped by the owner via email`, "warning");
      return;
    }
    if (wantsStop && !isPolling) {
      await sendEmail({ to: fromAddress, subject: `Re: ${subject}`, body: `${ownerGreeting()}\n\nI'm already paused.\n\n${CLARA_SIGNATURE}`, threadId: gmailThreadId, inReplyTo: messageId, references: messageId });
    ...
    if (wantsStart && !isPolling) {
      ...
      addLog(`▶️ Clara started by the owner via email`, "success");
      return;
    }
    if (wantsStart && isPolling) {
      await sendEmail({ to: fromAddress, subject: `Re: ${subject}`, body: `${ownerGreeting()}\n\nAlready running.\n\n${CLARA_SIGNATURE}`, threadId: gmailThreadId, inReplyTo: messageId, references: messageId });
    ...
    if (thread?.stage === "waiting_draft_approval") {
      const isSendApproval = /\b(send\s+it|yes[\s,]+send|go\s+ahead|looks\s+good[\s,]+send|approved?)\b/i.test(`${subject} ${body}`);
      if (isSendApproval) {
      ...
      // Not an approval — treat as a revised instruction, clear the pending state
      ...
      if ((!thread || thread.stage === "done" || thread.stage === "calendar_context") && activeSchedulingThreads.length > 0) {
        ...
        if (!matchedEntry) {
          ...
          const matchRaw = await askClaude(
            `${OWNER_NAME} sent a message to his Operations Lead Clara. There are active scheduling threads in progress.\n\n` +
...
${CLARA_SIGNATURE}`, threadId: gmailThreadId, inReplyTo: messageId, references: messageId });
      ...
      const intent = await askClaude(
        `${OWNER_NAME} sent a message to his Operations Lead Clara (may be in any language). Classify it as exactly one of:\n` +
        ...
        `C) Correcting, cancelling, or overriding something Clara already did\n\n` +
...
${SNIPPET_SCHEDULING}${tpHint}\n\n${OWNER_NAME} has offered these time slots: ${wrapUntrusted(slots)}\nWrite a short, warm email to ${thread.thirdPartyFirstName} proposing them and asking which works best.\nOpening: ${greeting}\nUse exact times as given. Write in ${lang}\nClosing: ${sig}\nWrite email body only`, 400, 1, MODEL_FAST);
    ...
    // The owner deliberately chose. To: usually just contains Clara's address.
    ...
    if (thirdPartyEmails.length) {
      ...
      saveThread(gmailThreadId, { stage: "waiting_for_slots", thirdPartyEmail: primaryEmail, thirdPartyEmails, thirdPartyFirstName: primaryName, thirdPartyNames: tpNames, isMultiple, originalSubject: subject, lastThirdPartyMessageId: messageId, thirdPartyGmailThreadId: gmailThreadId, ownerEmail: safeOwnerEmail(fromAddress), ownerGmailThreadId: gmailThreadId, isFirstContact: true, language: threadLanguage, thirdPartyLanguage: tpLang, ownerConfirmed: false, thirdPartyConfirmed: false });

      const ccIntent = await askClaude(`Was this email CCing Clara to schedule a meeting with the people copied?\nSubject: ${wrapUntrusted(subject)}\nBody: ${wrapUntrusted(truncate(body, 600))}\nReply YES or NO.`, 10, 1, MODEL_HAIKU);
      const ccBody = await askClaude(
        `${withRules(SNIPPET_OWNER_REPLY)}\n\nYou have been CC'd on an email by ${OWNER_NAME} with ${desc}.\n` +
        (ccIntent.trim().startsWith("YES")
          ? `The CC appears to be about scheduling a meeting with ${desc}. Write a short, natural reply to ${OWNER_NAME} asking for his available time slots for a meeting with ${desc}, the meeting format (call, Google Meet, or in-person), and the name to use for the calendar invite — all in one or two conversational sentences. Always mention the other party by name. Do not use any lists or numbers.`
      ...
      addLog(`✅ Asked the owner for slots`, "success");
      return;
    }
  ...
  if (tThread && (tThread.stage === "waiting_for_confirmation" || tThread.stage === "waiting_for_slots" || tThread.stage === "waiting_for_owner_confirmation")) {
    ...
    const check = await askClaude(
      ...
      `${tThread.thirdPartyAskedForSlots ? 'Note: Clara asked them for their availability — they are providing slots, not confirming a booking.\n\n' : ''}` +
  ...
  if (urgencyCheck.trim().startsWith("URGENT")) {
    ...
    await alertOwner(`🚨 Clara: Urgent email from ${fromName} — "${subject.slice(0, 80)}"`);
  }
...
${CLARA_SIGNATURE}`,
      ...
      await alertOwner(`💰 Clara: Invoice from ${expense.vendor} — ${amountStr}`);
  ...
  if (classification.startsWith("RESCHEDULE_REQUEST")) {
    ...
    const gBody = await askClaude(
      `You are ${CLARA_NAME}, PA to ${OWNER_NAME}. Write a short, clean message to ${OWNER_NAME} in English.\n\n` +
  ...
  if (classification.startsWith("MEETING_REQUEST")) {
    ...
    // not waiting for slots. This is the key fix for the Angi/11am scenario.
...
// ─── Email polling ────────────────────────────────────────────────────────────
let _fetchMutex = null; // promise-based mutex — prevents concurrent poll runs

async function fetchNewEmails() {
  if (_fetchMutex) { addLog("⏭️ Poll skipped — previous run still in progress", "info"); return; }
  ...
  let _releaseMutex;
  _fetchMutex = new Promise(resolve => { _releaseMutex = resolve; });
  try {
  ...
  const res   = await gmail.users.messages.list({ userId: "me", q: `to:${CLARA_EMAIL} after:${since}`, maxResults: 100 });
  ...
  for (const message of messages) {
    ...
    try {
      ...
      try {
        await sendEmail({
          to: OWNER_DEFAULT,
          subject: `⚠️ Clara: processing error`,
...
async function sendMorningBriefing() {
  ...
  try {
    ...
    const briefing = await askClaude(
      `You are ${CLARA_NAME}, PA to ${OWNER_NAME}. Write the morning gatekeeper email.\n\n` +
      `Today is ${todayStr}.\n\n` +
      `=== TODAY'S MEETINGS ===\n${calendarText}\n\n` +
      `=== TOMORROW'S MEETINGS ===\n${tomorrowText}\n\n` +
      `=== OPEN THREADS ===\n${threadsText}\n\n` +
    ...
    if (TELEGRAM_ENABLED && TELEGRAM_CHAT_ID) {
      try {
        const tgBriefing = await askClaude(
          `You are ${CLARA_NAME}, PA to ${OWNER_NAME}. Write a very short Telegram morning briefing.\n\n` +
          `Today is ${todayStr}.\n\n` +
          `=== TODAY'S MEETINGS ===\n${calendarText}\n\n` +
          `=== OPEN THREADS ===\n${threadsText}\n\n` +
          `=== AWAITING REPLIES ===\n${staleText}\n\n` +
...
async function chaseStaleThreads() {
  ...
  if (stale.length) {
    ...
    await alertOwner(`⏰ Clara: ${stale.length} thread${stale.length > 1 ? "s" : ""} with no reply for 5+ days: ${names}`);
  }
...
`;
        ...
        const brief = await askClaude(
          `You are ${CLARA_NAME}, PA to ${OWNER_NAME}. Write a concise meeting prep brief.\n\n` +
    ...
    if (romeWeekday === "Fri" && romeHour === 17 && romeMin === 0 && scheduleJobs._lastWeeklyDigest !== today) {
      ...
      try {
        ...
        if (TELEGRAM_ENABLED && TELEGRAM_CHAT_ID) {
          ...
          await sendTelegram(TELEGRAM_CHAT_ID, tgDigest).catch(e => addLog(`⚠️ Silent failure: ${e.message}`, "warning"));
        }
    ...
    // ── Automated follow-up system — daily at 09:00 Rome time ─────────────────
    if (romeHour === 9 && romeMin === 0 && scheduleJobs._lastFollowUpDate !== today) {
      scheduleJobs._lastFollowUpDate = today;
      try {
        // 1. Check scheduled follow-ups from the follow-up module
        const dueFollowUps = await followUp.checkDue(ctx);
        let autoSent = 0;
        for (const fu of dueFollowUps.slice(0, 5)) { // cap at 5 auto follow-ups per day
          try {
            const emailBody = await followUp.generateFollowUpEmail(ctx, fu);
            const lastThread = findThreadByEmail(fu.email, { requireActive: false });
            await sendEmail({
              to: fu.email,
              subject: `Re: ${fu.subject || "Following up"}`,
              body: emailBody,
              threadId: lastThread?.[1]?.thirdPartyGmailThreadId || null,
            });
            followUp.markSent(fu.id);
            autoSent++;
            addLog(`📬 Auto follow-up sent to ${fu.name} (${fu.email}) — attempt ${fu.attempts + 1}`, "success");
          } catch (e) {
            addLog(`⚠️ Auto follow-up to ${fu.name} failed: ${e.message}`, "warning");
          }
        }
        if (autoSent) addLog(`📬 ${autoSent} automated follow-up(s) sent`, "info");

        // 2. Proactive reminders for stale contacts (existing behavior)
        const remindersSent = [];
        for (const [email, p] of Object.entries(profiles)) {
          if (remindersSent.length >= 3) break;
          const cs = p.conversationState?.state;
          ...
          // Don't remind if we already have a scheduled follow-up
          const pendingFU = followUp.list().find(f => f.email === email);
          if (pendingFU) continue;

          const name = p.name || email;
          if (TELEGRAM_ENABLED && TELEGRAM_CHAT_ID) {
            await sendTelegram(TELEGRAM_CHAT_ID, `You haven't been in touch with ${name} for ${daysSince} days. Want me to follow up?`).catch(e => addLog(`⚠️ Silent failure: ${e.message}`, "warning"));
          }
          remindersSent.push(name);
        }
        if (remindersSent.length) addLog(`📬 Follow-up reminders sent for: ${remindersSent.join(", ")}`, "info");
      } catch (e) { addLog(`⚠️ Follow-up system error: ${e.message}`, "warning"); }
    }
...
// If GMAIL_PUBSUB_TOPIC is set, register a watch on Gmail that pushes new mail
// notifications to /gmail/push instead of relying on polling.
const GMAIL_PUBSUB_TOPIC = process.env.GMAIL_PUBSUB_TOPIC || ""; // e.g. projects/my-project/topics/gmail-push
let pushNotificationsActive = false;

async function setupGmailPush() {
  if (!GMAIL_PUBSUB_TOPIC) return false;
  try {
    ...
    addLog(`📡 Gmail push notifications active (expires: ${res.data.expiration})`, "success");
    // Gmail watch expires after 7 days — auto-renew daily
    const renewInterval = setInterval(async () => {
      try {
        await gmail.users.watch({ userId: "me", requestBody: { topicName: GMAIL_PUBSUB_TOPIC, labelIds: ["INBOX"] } });
        addLog("📡 Gmail push watch renewed", "info");
      } catch (e) { addLog(`⚠️ Gmail push watch renewal failed: ${e.message}`, "warning"); }
    }, 24 * 60 * 60 * 1000);
    if (renewInterval.unref) renewInterval.unref();
    return true;
  } catch (e) {
    addLog(`⚠️ Gmail push setup failed (falling back to polling): ${e.message}`, "warning");
    return false;
  }
}

// Pub/Sub webhook — Google sends POST with base64-encoded notification
app.post("/gmail/push", express.json(), async (req, res) => {
  res.sendStatus(200); // Acknowledge immediately
  try {
    const data = req.body?.message?.data;
    if (!data) return;
    const decoded = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
    addLog(`📡 Gmail push notification: historyId=${decoded.historyId}`, "info");
    // Trigger a fetch — the idempotency in fetchNewEmails prevents double-processing
    await fetchNewEmails();
  } catch (e) {
    addLog(`⚠️ Gmail push handler error: ${e.message}`, "warning");
  }
});
...
function startPolling() {
  if (isPolling) return;
  isPolling = true;
  // If push notifications are active, poll less frequently (every 5 min as backup)
  const interval = pushNotificationsActive ? Math.max(config.pollIntervalMinutes, 5) : config.pollIntervalMinutes;
  addLog(`🔄 Polling every ${interval} min${pushNotificationsActive ? " (push active — backup mode)" : ""}`, "success");
  const run = async () => {
    try { await fetchNewEmails(); } catch (e) { addLog(`❌ ${e.message}`, "error"); }
    if (isPolling) pollingTimer = setTimeout(run, interval * 60 * 1000);
  };
  run();
}
...
// Keeps last 20 messages so Clara has context for follow-up questions like
// "where did you send it?" or "what was the email about?"
const chatHistory = []; // [{ role: "owner"|"clara", text, time }]
...
function getChatContext() {
  ...
  return "\n=== RECENT CHAT HISTORY ===\n" +
    chatHistory.map(m => `[${m.role === "owner" ? OWNER_NAME : "Clara"}] ${m.text}`).join("\n") +
    "\n=== END CHAT HISTORY ===\n";
}
...
async function handleInboundMessage(body, replyFn) {
  ...
  try {
    const rawTasks = await parseInstructions(body, `Message from ${OWNER_NAME}`);
    if (!rawTasks?.length) {
    ...
    const msgLang2 = sanitiseLang(await detectLanguage(body));
    const summary = await askClaude(
      `${SNIPPET_IDENTITY}\n${MSG_STYLE}\n\n` +
      getChatContext() +
      `You just completed these tasks for ${OWNER_NAME}:\n` +
...
// ── Wire back-references onto ctx (defined above, set now that functions exist) ──
ctx.executeTask = executeTask;
ctx.handleInboundMessage = handleInboundMessage;
...
app.post(TELEGRAM_WEBHOOK_PATH, async (req, res) => {
  ...
  try {
    ...
    // Security: TELEGRAM_CHAT_ID must be set explicitly in env vars
    if (!TELEGRAM_CHAT_ID) {
      addLog(`⚠️ TELEGRAM_CHAT_ID not configured — message from ${from} (chat ${chatId}) ignored. Set TELEGRAM_CHAT_ID in Render env vars.`, "warning");
      await sendTelegram(chatId, `I'm not yet configured to accept messages. My owner needs to set the TELEGRAM_CHAT_ID environment variable.`);
      return;
    }
    ...
    if (text === "/start") {
      await sendTelegram(chatId, `Hello! ${CLARA_NAME} here. Send me anything — tasks, questions, files, or just say hi.`);
      return;
    }
    ...
    if (TELEGRAM_CHAT_ID) await sendTelegram(TELEGRAM_CHAT_ID, `Sorry, I hit a problem: ${e.message}`).catch(e => addLog(`⚠️ Silent failure: ${e.message}`, "warning"));
  }
});
...
app.get("/doc/:id", apiLimiter, async (req, res) => {
  ...
  alertOwner(viewLabel).catch(e => addLog(`⚠️ Silent failure: ${e.message}`, "warning"));
...
app.get("/auth/callback", async (req, res) => {
  ...
  const label  = isContacts ? "Contacts (Owner)" : isCalendar ? "Calendar (Owner)" : "Gmail (Clara)";
  ...
  try {
    ...
    if (tokens.refresh_token) {
      addLog(`🔑 New ${label} refresh token obtained — save ${envVar} in Render env vars via dashboard`, "success");
      // Token intentionally NOT logged — retrieve from OAuth callback response or Render dashboard
    }
    res.send(`<html><body style="font-family:monospace;padding:40px;background:#1a1714;color:#f5f0e8;"><h2 style="color:#b8965a">✅ ${label} authorized!</h2>${tokens.refresh_token ? `<p>A new refresh token was obtained.</p><p style="background:#2a2520;padding:12px;border:1px solid #b8965a;word-break:break-all;"><code>${envVar}=${tokens.refresh_token}</code></p><p>Copy the above and save it in your <strong>Render environment variables</strong>.</p>` : "<p>Token already stored.</p>"}<p><a href="/" style="color:#b8965a">← Back to Clara</a></p></body></html>`);
  } catch (e) {
...
// ─── NANDA: Discoverability — Agent Card endpoint (public, no auth) ──────────
const { AGENT_CARD } = require("./index.js");
app.get("/agent-card", apiLimiter, (req, res) => {
  res.json({
    ...AGENT_CARD,
    runtime: {
      ...
      googleAuthorized: config.isAuthorized,
      isPolling,
    },
  });
});
app.get("/.well-known/agent.json", apiLimiter, (req, res) => {
  res.json(AGENT_CARD);
});
...
// Short aliases used by the dashboard
app.post("/api/start", apiLimiter, requireAuth, (req, res) => { startPolling(); res.json({ ok: true }); });
app.post("/api/stop",  apiLimiter, requireAuth, (req, res) => { stopPolling();  res.json({ ok: true }); });
app.post("/api/poll",  apiLimiter, requireAuth, async (req, res) => { try { await fetchNewEmails(); res.json({ ok: true }); } catch (e) { res.status(500).json({ error: "Poll failed." }); } });
...
app.get("/api/expenses/export/zip", apiLimiter, requireAuth, async (req, res) => {
  try {
    ...
    res.setHeader("Content-Disposition", `attachment; filename="clara_expenses_${dateStr}.zip"`);
    res.send(zipBuf);
  } catch (e) {
...
function profileToVCard(email, profile) {
  ...
  lines.push("NOTE:From Clara");
  // UID for deduplication on re-import
  lines.push(`UID:clara-crm-${email.replace(/[^a-z0-9@._-]/gi, "")}`);
...
app.get("/api/contacts/export.vcf", apiLimiter, requireAuth, (req, res) => {
  const cards = Object.entries(profiles)
    .filter(([email, p]) => !isCrmBlocked(email) && !isOwner(email) && email !== CLARA_EMAIL.toLowerCase())
  ...
  res.setHeader("Content-Disposition", 'attachment; filename="clara_contacts.vcf"');
  res.send(vcf);
});
...
let contactsSyncMap = cryptoStore.readEncrypted(CONTACTS_SYNC_FILE, {}); // { email: { resourceName, etag, lastSync } } — encrypted at rest
function saveContactsSync() { cryptoStore.atomicWriteEncrypted(CONTACTS_SYNC_FILE, contactsSyncMap).catch(e => console.error(e.message)); }
...
function getContactsOAuth() {
  ...
  const contactsToken = process.env.GOOGLE_CONTACTS_REFRESH_TOKEN;
...
async function syncContactToGoogle(email, profile) {
  ...
  const personBody = {
    ...
    biographies: [{ value: "From Clara", contentType: "TEXT_PLAIN" }],
  };
...
app.post("/api/contacts/sync-google", apiLimiter, requireAuth, async (req, res) => {
  ...
  if (!auth) {
    ...
    try {
      ...
      await sendEmail({
        to: OWNER_DEFAULT,
        subject: "Clara — Authorize Google Contacts sync",
  ...
  const eligible = Object.entries(profiles)
    .filter(([email]) => !isCrmBlocked(email) && !isOwner(email) && email !== CLARA_EMAIL.toLowerCase() && !crmDeleted.has(email));
...
// ─── NANDA: Audit Log endpoint ───────────────────────────────────────────────
app.get("/api/audit", apiLimiter, requireAuth, (req, res) => {
  const { action, target, limit: lim } = req.query;
  let results = auditLog;
  if (action) results = results.filter(e => e.action === action);
  if (target) results = results.filter(e => e.target && e.target.toLowerCase().includes(target.toLowerCase()));
  const max = Math.min(parseInt(lim) || 200, 2000);
  res.json(results.slice(-max));
});

// ── Institutional: Approval Management ──────────────────────────────────────
app.get("/api/approvals/pending", apiLimiter, requireAuth, (req, res) => {
  if (!_ic) return res.json({ ok: true, pending: [] });
  res.json({ ok: true, pending: _ic.approval.getPending() });
});

app.post("/api/approvals/:id/approve", apiLimiter, requireAuth, (req, res) => {
  if (!_ic) return res.status(500).json({ ok: false, error: 'Institutional core not loaded' });
  const result = _ic.approval.approve(req.params.id, req.body.approvedBy || 'owner', req.body.notes || '');
  res.json({ ok: result.success, ...result });
});

app.post("/api/approvals/:id/reject", apiLimiter, requireAuth, (req, res) => {
  if (!_ic) return res.status(500).json({ ok: false, error: 'Institutional core not loaded' });
  const result = _ic.approval.reject(req.params.id, req.body.rejectedBy || 'owner', req.body.reason || '');
  res.json({ ok: result.success, ...result });
});
...
app.get("/api/templates", apiLimiter, requireAuth, (req, res) => res.json(templates.list()));
app.post("/api/templates", apiLimiter, requireAuth, (req, res) => {
  try {
    const tpl = templates.create(req.body);
    addLog(`📋 Template created: "${tpl.name}"`, "info");
    res.json(tpl);
  } catch (e) { res.status(400).json({ error: e.message }); }
});
app.delete("/api/templates/:id", apiLimiter, requireAuth, (req, res) => {
  if (templates.remove(req.params.id)) res.json({ ok: true });
  else res.status(404).json({ error: "Template not found" });
});
app.post("/api/templates/:id/render", apiLimiter, requireAuth, (req, res) => {
  const tpl = templates.get(req.params.id);
  if (!tpl) return res.status(404).json({ error: "Template not found" });
  const { email, ...vars } = req.body;
  const profile = email ? profiles[email.toLowerCase()] : null;
  const rendered = templates.render(tpl, vars, profile);
  res.json(rendered);
});

// ── Follow-up API routes ─────────────────────────────────────────────────────
app.get("/api/follow-ups", apiLimiter, requireAuth, (req, res) => res.json(followUp.list()));
app.post("/api/follow-ups", apiLimiter, requireAuth, (req, res) => {
  try {
    const fu = followUp.schedule(req.body);
    addLog(`📬 Follow-up scheduled for ${fu.name} in ${fu.delayDays} days`, "info");
    res.json(fu);
  } catch (e) { res.status(400).json({ error: e.message }); }
});
app.delete("/api/follow-ups/:id", apiLimiter, requireAuth, (req, res) => {
  const cancelled = followUp.cancel(null, null); // cancel by ID not supported yet
  res.json({ ok: true, cancelled });
});

// ── Pipeline summary API ─────────────────────────────────────────────────────
app.get("/api/pipeline", apiLimiter, requireAuth, (req, res) => {
  const stages = {};
  let totalValue = 0, totalWeightedValue = 0;
  for (const [email, p] of Object.entries(profiles)) {
    if (!p.pipeline) continue;
    const stage = p.pipeline.stage || "cold_lead";
    if (!stages[stage]) stages[stage] = { count: 0, contacts: [], totalValue: 0 };
    stages[stage].count++;
    stages[stage].contacts.push({ email, name: p.name, value: p.pipeline.value, probability: p.pipeline.probability });
    if (p.pipeline.value) {
      totalValue += p.pipeline.value;
      totalWeightedValue += p.pipeline.value * (p.pipeline.probability / 100);
    }
  }
  res.json({ stages, totalValue, totalWeightedValue, totalContacts: Object.keys(profiles).length });
});

// ── Calendar summary API (for dashboard) ─────────────────────────────────────
app.get("/api/calendar/upcoming", apiLimiter, requireAuth, async (req, res) => {
  try {
    const now = new Date();
    const end = new Date(now.getTime() + 7 * 24 * 60 * 60 * 1000);
    const events = await fetchCalendarEvents({ timeMin: now.toISOString(), timeMax: end.toISOString(), maxResults: 20, includeAll: false });
...
(function validateEnv() {
  const required = [
    ...
    { key: "CLARA_EMAIL",            hint: "Clara's Gmail address — the account she sends emails from" },
  ];
  const optional = [
